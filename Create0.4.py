...
def api1(password, first_name, last_name, birthday):
    ...
    if id:
        ...
        if 'Locked' in check:
            ...
            success.append(id)
            open("/sdcard/AUTO-BRYX/ERROR-FB-REGISTER-TOKEN.txt","a").write(id+f"|{email}|{password}|{token}\n")
            open("/sdcard/AUTO-BRYX/fname.txt","a").write(first_name+"\n")
            open("/sdcard/AUTO-BRYX/lname.txt","a").write(last_name+"\n")
            open("/sdcard/AUTO-BRYX/ERROR-FB-REGISTER-OK.txt","a").write(email+f"|{password}\n")
    else:
